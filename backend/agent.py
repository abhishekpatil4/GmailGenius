...
#     gmail_agent = Agent(
#         role="Google Agent",
#         goal="""You take action on Gmail using Gmail APIs & you can extract useful information from documents and store them into an excel sheets""",
#         backstory="""You are an AI agent responsible for taking actions on Google\'s gmail and sheets on users' behalf. 
#         You need to take action on Gmail using Gmail's APIs. Use correct tools to run APIs from the given tool-set. You can extract useful information from documents and Store them in Excel sheets""",
#         verbose=True,
#         tools=tools,
#         llm=llm,
#         cache=False,
...
#         agent=gmail_agent,
#         expected_output="Store the information in sheets",
#     )
...
def run_crew(emailKeywords: str, attributes: str):
    try:
        ...
        crew = Crew(agents=[gmail_agent], tasks=[task])
        result = crew.kickoff()
        return result, 200  # Return the result and status code
